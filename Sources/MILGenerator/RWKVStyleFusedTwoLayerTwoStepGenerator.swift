...
/// Two-token exact branch-preparation path with two recurrent layers fused into one MIL program.
///
/// Inputs:
/// - `x0`: token 0 activation
/// - `x1`: token 1 activation
/// - `stateIn0`: layer 0 incoming recurrent state
/// - `stateIn1`: layer 1 incoming recurrent state
///
/// Outputs:
/// - `x0Next`: activation after token 0 has flowed through both layers
/// - `x1Next`: activation after token 1 has flowed through both layers
/// - `stateMid0`: layer 0 state after token 0
/// - `stateMid1`: layer 1 state after token 0
/// - `stateOut0`: layer 0 state after token 1
/// - `stateOut1`: layer 1 state after token 1
public struct RWKVStyleFusedTwoLayerTwoStepGenerator: MILProgramGenerator {
    ...
    public var milText: String {
        ...
        b.appendLine(
            "    func main<ios18>(tensor<fp16, [1, \(dim), 1, \(lane)]> x0, tensor<fp16, [1, \(dim), 1, \(lane)]> x1, tensor<fp16, [1, \(dim), 1, \(lane)]> stateIn0, tensor<fp16, [1, \(dim), 1, \(lane)]> stateIn1) {"
        )
        ...
        b.appendLine("    } -> (x0Next,x1Next,stateMid0,stateMid1,stateOut0,stateOut1);")
        b.appendLine("}")
        return b.text
    }
